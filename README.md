...
We have already downloaded an OSM extract created with [slice.openstreetmap.us](https://slice.openstreetmap.us/).
...
Examine the catalog endpoint by appending `/catalog` to the URL. See that `workshop` source is available.
...
```
martin /data/workshop.mbtiles /data/benches.mbtiles
```

In maputnik, add the new source using `https://{public URL for your Martin instance}/benches` - lets call it `benches`. You can then add a new layer to the map using the `benches` source. For example, you can add a circle layer that shows benches in workshop.
